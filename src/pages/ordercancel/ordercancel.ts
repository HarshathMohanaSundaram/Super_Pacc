...
export class OrdercancelPage {
  ...
  cancel() {
    ...
    let url = "http://demo.sputnikinfotech.com/sputi/ion/index.php/api/food_cancel";
    console.log(url);
    let loading = this.loadingCtrl.create({
    });
    loading.present();
    let body = JSON.stringify({ appkey: "t@ftc2o18",  order_id: this.orderid, feed: 'Cancelled' });
      ...
      .subscribe(
        data => {
          ...
          if (dataval.status == 'error') {
            ...
            this.presentToast('Your Order Cancelled');
